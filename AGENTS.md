...
- Questions authorize investigation and explanation only. Do not modify files, install tools, or run builds/tests unless requested. If the solution is obvious, just propose it.
...
- Avoid silencing errors and warnings. Handle or propagate errors; log where they are handled.
...
- `jq` for JSON
- `yq` for YAML
 If a preferred tool is unavailable, install it. If it's during question-only investigation - ask permission first.
